...
# ── By agency ──────────────────────────────────────────────────────
st.markdown("### 🏢 By Awarding Top-Tier Agency (rolls Navy/Army/Air Force into DoD)")
# Use Toptier field if present; fallback to Awarding Agency
agency_col = "Awarding Agency Toptier" if "Awarding Agency Toptier" in df.columns else "Awarding Agency"
by_agency = (df.groupby(agency_col)
             .agg(awards=("Award ID", "count"),
                  total=("Award Amount", "sum"))
             .sort_values("total", ascending=False))
...
for i, row in df.nlargest(25, "Award Amount").iterrows():
    ...
    with st.expander(
        ...
        c1.markdown(f"**Top-Tier Agency:** {row.get('Awarding Agency Toptier', row.get('Awarding Agency', ''))}")
