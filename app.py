...
# ── By agency ──────────────────────────────────────────────────────
st.markdown("### 🏢 By Parent Agency (Top-Tier)")
by_agency = (df.groupby("Awarding Agency Toptier")
             .agg(awards=("Award ID", "count"),
                  total=("Award Amount", "sum"))
             .sort_values("total", ascending=False))
...
for i, row in df.nlargest(25, "Award Amount").iterrows():
    ...
    with st.expander(
        ...
        f"{row.get('Awarding Agency Toptier', '')[:30]}",
        expanded=False,
    ):
        c1, c2 = st.columns([1, 1])
        c1.markdown(f"**Award ID:** `{row.get('Award ID', '')}`")
        c1.markdown(f"**Recipient UEI:** `{row.get('Recipient UEI', '')}`")
        c1.markdown(f"**Parent Agency (Top-Tier):** {row.get('Awarding Agency Toptier', '')}")
